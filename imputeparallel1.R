...
flag60 = create.flag(data[1:7*2000,-c(1:3)], window=60)
label <- data[1:7*2000,1:2]
...
results <- foreach(i=1:5, .export=c('accel.impute'), .packages='accelmissing') %dopar% {  
  accel.impute(PA=data[1:7*2000,-c(1:3)], label=label, flag=flag60, demo=demo[1:2000,],
               method="zipln", time.range=c("09:00","20:59"), m=1,seed=seeds[i])
}


save(results,file="impute1.RData")