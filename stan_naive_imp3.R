...
imp1 <- read.csv("epi/NHANES_accel_imp3.csv")
...
mu[i,1] = alphaw[4]-alphaw[1]/(1+exp(-alphaw[2]*(Tstar[i]-alphaw[3])));
mu[i,2] = alphag[4]-alphag[1]/(1+exp(-alphag[2]*(Tstar[i]-alphag[3])));
mu[i,3] = alphat[4]-alphat[1]/(1+exp(-alphat[2]*(Tstar[i]-alphat[3])));
mu[i,4] = alphabs[4]-alphabs[1]/(1+exp(-alphabs[2]*(Tstar[i]-alphabs[3])));
mu[i,5] = alphal[1] + alphal[2]*(Tstar[i]) + alphal[3]*pow(Tstar[i],2);
mu[i,6] = alphabd[1] + alphabd[2]*(Tstar[i]) + alphabd[3]*pow(Tstar[i],2);
mu[i,7] = alphah[1] + alphah[2]*(Tstar[i]);
...
#summary(rs)
save(rs,file="/ptmp/dcries/stanout_naive_imp3.RData")
