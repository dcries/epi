...
out7 = mcmc_epi_mixture(MetS,tstar2, start, prior, K,200000,50000,thin=10)
out7$dic
pmat <- array(0,dim=c(nrow(out7$beta),nrow(MetS),K))
for(i in 1:K){
  for(j in 1:nrow(MetS)){
    pmat[,j,i] <- out7$pmat[j,i,]
  }
}
out7$pmat <- NULL
...
out7=list(out7,permutations)

save(out7,file="/ptmp/dcries/stanout_realmix7.RData")
