...
out8 = mcmc_epi_mixture(MetS,tstar2, start, prior, K,300000,50000,thin=10)
out8$dic
pmat <- array(0,dim=c(nrow(out8$beta),nrow(MetS),K))
for(i in 1:K){
  for(j in 1:nrow(MetS)){
    pmat[,j,i] <- out8$pmat[j,i,]
  }
}
out8$pmat <- NULL
...
out8=list(out8=out8,permutations=permutations)

save(out8,file="/ptmp/STAT/dcries/stanout_mix8.RData")
